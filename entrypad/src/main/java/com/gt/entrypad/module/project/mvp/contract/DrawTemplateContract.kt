 ...
 * 功能：
 */
interface DrawTemplateContract {
    //对于经常使用的关于UI的方法可以定义到IView中,如显示隐藏进度条,和显示文字消息
    interface View : IView {
        fun onTemplateListResult(tempalteList : NormalList<DrawTemplateBean>)
    ...
    interface Model : IModel{
        fun templateListData(requestBody: RequestBody) : Observable<NormalList<DrawTemplateBean>>
    }
	
	//方法
	abstract class Presenter : BasePresenter<View, Model>(){
        abstract fun getTemplateList(requestBody: RequestBody)
        abstract fun downloadTemplate(templateBean: DrawTemplateBean)
