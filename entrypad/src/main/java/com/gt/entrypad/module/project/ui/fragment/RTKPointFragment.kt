        ...
        btnGraphic.setOnClickListener {
            ...
            if (rtkData.size == 3) {
                ...
                val rtkInfoBean = RTKTool.rtkActualLocation(
                    RtkInfoBean(
                        rtkPointBean1.point.x.toDouble(),
                        rtkPointBean1.point.y.toDouble(),
                        rtkPointBean1.distance
                    ),
                    RtkInfoBean(
                        rtkPointBean2.point.x.toDouble(),
                        rtkPointBean2.point.y.toDouble(),
                        rtkPointBean2.distance
                    ),
                    RtkInfoBean(
                        rtkPointBean3.point.x.toDouble(),
                        rtkPointBean3.point.y.toDouble(),
                        rtkPointBean3.distance
                    )
                )
                rtkData.forEach {
                    it.sitePoint =
                        PointF(rtkInfoBean.pointX.toFloat(), rtkInfoBean.pointY.toFloat())
                }
