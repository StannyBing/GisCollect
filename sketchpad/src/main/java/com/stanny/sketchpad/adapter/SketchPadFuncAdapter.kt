...
class SketchPadFuncAdapter(var dataList: List<SketchPadFuncBean>) :
    ...
    override fun convert(helper: ZXBaseHolder, item: SketchPadFuncBean) {
        ...
        if (item.isChecked) helper.setTextColor(R.id.tv_func_name,Color.parseColor("#17abe3")) else helper.setTextColor(R.id.tv_func_name,ContextCompat.getColor(mContext,R.color.gray_cc))
    }
}