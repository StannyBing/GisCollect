...
import com.gt.giscollect.tool.CameraView.util.AngleUtil;
import com.gt.giscollect.tool.CameraView.util.CameraParamUtil;
import com.gt.giscollect.tool.CameraView.util.CheckPermission;
...
public class CameraInterface implements Camera.PreviewCallback {
    ...
    //视频质量
    private int mediaQuality = GTCameraView.MEDIA_QUALITY_MIDDLE;
        ...
        if (mCamera != null) {
            try {
                ...
                Camera.Size pictureSize = CameraParamUtil.getInstance().getPictureSize(mParams
                        .getSupportedPictureSizes(), 2000, screenProp);
...
//                mParams.setPreviewSize(ZXScreenUtil.getScreenHeight(), ZXScreenUtil.getScreenWidth());
...
//
    ...
    public void startRecord(Surface surface, float screenProp, ErrorCallback callback) {
        ...
        if (mParams.getSupportedVideoSizes() == null) {
            videoSize = CameraParamUtil.getInstance().getPreviewSize(mParams.getSupportedPreviewSizes(), 2000,
                    screenProp);
        } else {
            videoSize = CameraParamUtil.getInstance().getPreviewSize(mParams.getSupportedVideoSizes(), 2000,
                    screenProp);
        }
