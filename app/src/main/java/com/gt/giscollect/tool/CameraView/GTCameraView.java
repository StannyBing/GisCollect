package com.gt.giscollect.tool.CameraView;
...
import com.gt.giscollect.tool.CameraView.AlbumView;
import com.gt.giscollect.tool.CameraView.CameraInterface;
import com.gt.giscollect.tool.CameraView.CaptureLayout;
import com.gt.giscollect.tool.CameraView.FoucsView;
import com.gt.giscollect.tool.CameraView.listener.BtnClickListener;
import com.gt.giscollect.tool.CameraView.listener.CameraListener;
import com.gt.giscollect.tool.CameraView.listener.CaptureListener;
import com.gt.giscollect.tool.CameraView.listener.TypeListener;
import com.gt.giscollect.tool.CameraView.state.CameraMachine;
import com.gt.giscollect.tool.CameraView.view.CameraView;
 ...
 * =====================================
 */
public class GTCameraView extends FrameLayout implements CameraInterface.CameraOpenOverCallback, SurfaceHolder
...
//    private static final String TAG = "JCameraView";
    ...
    public static final int MEDIA_QUALITY_POOR = 2 * 1024*1024;
    public static final int MEDIA_QUALITY_FUNNY = 1 * 1024*1024;
    ...
    public GTCameraView(Context context) {
        this(context, null);
    }

    public GTCameraView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public GTCameraView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        mContext = context;
        //get AttributeSet
        iconSize = ZXSystemUtil.dp2px(30);
        iconMargin = ZXSystemUtil.dp2px(20);
        iconSrc = com.zx.zxutils.R.drawable.ic_camera;
        duration = 10 * 1000;       //没设置默认为10s
        initData();
        initView();
    }

    public GTCameraView setMaxVedioDuration(int maxDurationSecend) {
...
//        });
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        ...
        new Thread() {
            @Override
            public void run() {
                CameraInterface.getInstance().doOpenCamera(GTCameraView.this);
            }
        }.start();
    }
...
//                    Log.i("CJT", "result = " + (result - firstTouchLength));
    ...
    public GTCameraView setSaveVideoPath(String path) {
        CameraInterface.getInstance().setSaveVideoPath(path);
        return this;
    }

    public GTCameraView showAlbumView(boolean showAlbum) {
        this.mCaptureLayout.showAlbum(showAlbum);
        return this;
    }


    public GTCameraView setCameraLisenter(CameraListener jCameraLisenter) {
...
//    }
    ...
    public GTCameraView setCameraMode(int state) {
        this.mCaptureLayout.setButtonFeatures(state);
        return this;
    }

    //设置录制质量
    public GTCameraView setMediaQuality(int quality) {
...
//        }
    ...
    public void playVideo(Bitmap firstFrame, final String url) {
        videoUrl = url;
        GTCameraView.this.firstFrame = firstFrame;
    ...
    private void setFlashRes() {
        switch (type_flash) {
            case TYPE_FLASH_AUTO:
                mFlashLamp.setImageResource(com.zx.zxutils.R.drawable.ic_flash_auto);
                machine.flash(Camera.Parameters.FLASH_MODE_AUTO);
                break;
            case TYPE_FLASH_ON:
                mFlashLamp.setImageResource(com.zx.zxutils.R.drawable.ic_flash_on);
                machine.flash(Camera.Parameters.FLASH_MODE_ON);
                break;
            case TYPE_FLASH_OFF:
                mFlashLamp.setImageResource(com.zx.zxutils.R.drawable.ic_flash_off);
