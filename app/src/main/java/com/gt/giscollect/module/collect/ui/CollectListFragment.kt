...
//                                showToast("正在建设中")
                ...
                if (geoPackage.loadStatus == LoadStatus.LOADED) {
                    ...
                    geoTables.forEach { table ->
                        ...
                        featureLayer.addDoneLoadingListener {
                            ...
                            temporaryResetGeometey(featureLayer)
    ...
    //TODO 仅用于当前版本，下版本必须删除
    private fun temporaryResetGeometey(featureLayer: FeatureLayer) {
        try {
            if (ZXSystemUtil.getVersionCode() <= 12) {
                val query = featureLayer.featureTable.queryFeaturesAsync(QueryParameters())
                query.addDoneListener {
                    query.get().toList().forEach { feature ->
                        AppInfoManager.appInfo?.identifystyle?.forEach style@{
                            val obj = JSONObject(it)
                            if (obj.getString("itemName") == feature?.featureTable?.tableName) {
                                if (obj.has("size")) {
                                    val sizeKey = obj.getString("size")
                                    val wkidKey = obj.optInt("wkid", 0)
                                    if (feature?.attributes?.containsKey(sizeKey) == true) {
                                        feature.attributes?.set(
                                            sizeKey,
                                            DecimalFormat("#0.00").format(GeometrySizeTool.getArea(feature.geometry, wkidKey)).toDouble()
                                        )
                                        if (feature.featureTable?.canUpdate(feature) == true) {
